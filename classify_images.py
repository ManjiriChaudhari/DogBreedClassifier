...
def classify_images(images_dir, results_dic, model):
    ...
    classified the pet images of dalmatians.
     PLEASE : This function uses the classifier() function defined in 
    ...
    for filename in results_dic:
        ...
        # 2. Get raw classifier label
        classifier_label = classifier(full_path, model)

        # 3. Normalize classifier output
        classifier_label = classifier_label.lower().strip()

        # 4. Get the true pet label
        pet_label = results_dic[filename][0]

        # 5. Compare: check if pet_label is anywhere in classifier_label
